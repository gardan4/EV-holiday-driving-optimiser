 ...
 *
 * Two things that matter in the copy. **We say we don't know what's there** —
 ...
 * actually see whether there is a building next to it. And **the cost is to
 * the destination**, not to the stop: swapping a charger re-optimises every
 * stop after it, and a per-leg number would flatter the swap.
 */
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs leading-relaxed text-ink-500">
            Priced against arriving, at {Math.round(data.speed_kph)} km/h. We
            don&apos;t know what&apos;s at any of them — the charger data has no
            amenities — so open one in Maps if you need somewhere to eat.
          </p>
        </div>
        <button
          onClick={onClose}
    ...
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        ...
        <div className="mt-0.5 truncate text-xs text-ink-500">
          ...
          {Math.round(alt.power_kw)} kW · in {fmtKm(alt.dist_from_here_m)}
        </div>
