...
/**
 * Minutes to charge `socFrom` → `socTo`. Midpoint-rule integral over the
 * piecewise-linear curve, as in the simulator.
 *
 * With no `siteKw` this is the CAR's own ceiling, which is what the `/ev`
 * pages publish. Pass one and the site becomes the limit where it is lower —
 * a 50 kW post holds a 250 kW car at 50 — which is what the live drive needs
 * to answer "how much longer am I plugged in here".
 *
 * The curve is measured at the CABLE and `usable_kwh` is battery-side, so the
 * delivered power is derated by `charge_efficiency` AFTER the site cap and
 * before it is divided into battery energy — exactly as
 * `simulator.charge_minutes` does, in that order: a car pinned at a 50 kW post
 * still banks only ~93% of it. Without the derating these pages publish charge
 * times ~7% faster than the planner and than any test.
 */
export function chargeMinutes(
  ...
  while (soc < socTo - 1e-9) {
    ...
    const curve = curvePowerKw(v, soc + step / 2)
    const capped = siteKw && siteKw > 0 ? Math.min(curve, siteKw) : curve
    const p = Math.max(capped * eff, 1)
